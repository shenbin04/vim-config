...
autocmd FileType fzf tnoremap <buffer> <C-k> <Up>|
      ...
      \ tnoremap <buffer> <C-h> <C-\><C-n>:silent! call PrepareFZFSwitch()<CR>:call fzf#vim#history(fzf#vim#with_preview({'options': ['--query', @s]}), 1)<CR>|
      \ tnoremap <buffer> <C-b> <C-\><C-n>:silent! call PrepareFZFSwitch()<CR>:call fzf#vim#buffers('.', fzf#vim#with_preview({'options': ['--query', @s]}), 1)<CR>|
      \ tnoremap <buffer> <C-c> <C-\><C-n>:silent! call PrepareFZFSwitch()<CR>:call fzf#vim#gitfiles(expand('%:h'), fzf#vim#with_preview({'options': ['--query', @s, '--prompt', 'Dir> ']}), 1)<CR>|
      \ tnoremap <buffer> <C-a> <C-\><C-n>:silent! call PrepareFZFSwitch()<CR>:call fzf#vim#files('.', fzf#vim#with_preview({'options': ['--query', @s, '--prompt', 'Dir All> ']}), 1)<CR>
